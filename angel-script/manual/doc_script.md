...
`lvalue` должно быть выражением, которое вычисляется в ячейку памяти, где может быть сохранен результат вычислений, например переменной. Результат присваивания имеет тот же тип и то же значение, что и сохраняеммые данные. Выражение справа всегда вычесляется перед выражением слева.
...
Типы, не поддерживающие `object handles`, могут сконвертированы используя `value cast`. При этом будет создано новое значение, а в случае объектов - новый экземпляр класса.
